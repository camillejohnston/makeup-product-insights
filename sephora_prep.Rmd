...
sephora_token_counts_category <- sephora_tokens %>%
  filter((secondary_category %in% c("Moisturizers", "Treatments", "Cleansers", "Eye Care", "Masks", "Sunscreen"))) %>% ## keep only categories with fair amount of products
  group_by(word, year, secondary_category) %>%
  summarise(n = n(),
            average_rating = mean(rating, na.rm = TRUE),
            average_recommendation = mean(is_recommended, na.rm = TRUE)) %>% 
  arrange(-n)%>% 
...
fwrite(sephora, paste0(path, "/data/sephora.csv"))
